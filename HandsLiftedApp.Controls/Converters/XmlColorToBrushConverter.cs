...
namespace HandsLiftedApp.Controls.Converters
{
    ...
    /// </summary>
    public class XmlColorToBrushConverter : IValueConverter
    {
        /// <summary>
        ...
        {
            if (targetType == typeof(IBrush) && value is XmlColor xc)
            {
                var x = (Color)xc;
                return new ImmutableSolidColorBrush(x);
            }
        ...
        public static object? ConvertBack(object? value, Type? targetType)
        {
            ...
            {
                if (targetType == typeof(XmlColor))
                {
                    return new XmlColor(brush.Color);
                }
