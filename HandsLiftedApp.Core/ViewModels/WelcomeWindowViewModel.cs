...
namespace HandsLiftedApp.Core.ViewModels
{
    public class WelcomeWindowViewModel : ReactiveObject
    {
        ...
        public WelcomeWindowViewModel(MainViewModel parent)
        {
            ...
            if (parent.settings.LastOpenedPlaylistFullPath != null) {
                RecentPlaylists.Add(new RecentPlaylistEntry() { FilePath = parent.settings.LastOpenedPlaylistFullPath, FileName = Path.GetFileName(parent.settings.LastOpenedPlaylistFullPath) });
            }
        }
        ...
        {
            get => _recentPlaylists; set => this.RaiseAndSetIfChanged(ref _recentPlaylists, value);
        }
