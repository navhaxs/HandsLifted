...
| `Avalonia.Diagnostics` | `AvaloniaUI.DiagnosticsSupport` | `2.2.3` | **Resolved 2026-09-12.** The old in-process F12 DevTools window (`this.AttachDevTools()` in ~13 `Window` constructors) doesn't exist in the new model — `AvaloniaUI.DiagnosticsSupport` is an out-of-process bridge: one app-level `this.AttachDeveloperTools()` call in `App.Initialize()` ([App.axaml.cs](HandsLiftedApp.Core/App.axaml.cs)), and a separate standalone `AvaloniaUI.DeveloperTools` viewer app (installed independently, not a project package) attaches to the running app instead of opening in-process. Removed the 6 active per-window `AttachDevTools()` calls (`AboutWindow`, `MessageWindow`, `SongEditorWindow`, and 3 confirmation windows — `GroupItemsEditor`'s was already commented out, left alone) since attach is now app-wide, not per-window. Package renamed in `Directory.Packages.props` plus the 4 csproj that reference it (`Desktop`, `Core`, `Data`, `Controls`), still Debug-only via the same `Condition`. Verified: `dotnet restore` and `dotnet build -c Debug` both clean (0 errors) against Avalonia 12.1.1. **Still needs an interactive check**: install the standalone `AvaloniaUI.DeveloperTools` tool and confirm it actually attaches/inspects at runtime — the build only proves it compiles and loads, not that the diagnostics bridge itself works end-to-end.
