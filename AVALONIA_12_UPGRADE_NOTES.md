...
7. **`ExtendClientAreaChromeHints`** — fully removed API, 13 in-scope files, attribute deleted (matches the `BrowserWindow.axaml` reference pattern already in place). Accepted minor title-bar cosmetic regression on 4 of those windows (`AboutWindow`, `AddItemWindow`, `MainWindow`, `SetupWindow` — they combine `ExtendClientAreaToDecorationsHint="True"` without `SystemDecorations="None"`/`WindowDecorations="None"`, so may show doubled caption-button chrome — flagged for the visual pass, not fixed).
...
- Visual smoke-test checklist once the app can be run interactively: title-bar chrome on `AboutWindow`/`AddItemWindow`/`MainWindow`/`SetupWindow` (see item 7 above); `LivePane`'s live-outputs flyout position and `SongArrangementControl`'s stanza context-menu position (item 10 — a property rename whose visual equivalence is asserted from source, not yet demonstrated by running the app); the 4 drag sources touched by item 3 (library pane, library query, media-group thumbstrip, add-item drop target).
